...
function Store() {
  ...
  let handelDelete = (el)=>{
    ...
    if(isauth()){
      ...
      axios.delete(`${bisUrl}/office/stores/${el.id}`,config).then(()=>{
        let _data= data;
  ...
  return (
    ...
    <h6 className='text-dark'><FontAwesomeIcon icon={faStore} /> إدارة المخازن </h6>
    ...
    <div className='row g-1 mt-3'>
      ...
      {
          check_permissions("office.add_store") ?  <div className='col-12 col-lg-2 col-md-3 col-sm-12'>
          <Link to={'add'} className="btn btn-dark btn-sm w-100" style={{fontSize:'14px'}} role="button">إضافة مخزن <FontAwesomeIcon icon={faAdd} /></Link>
          </div> : <div className='col-12 col-lg-2 col-md-3 col-sm-12'>
          <Link className="btn btn-secondary btn-sm w-100" style={{fontSize:'14px',cursor:"not-allowed"}} role="button">إضافة مخزن <FontAwesomeIcon icon={faAdd} /></Link>
          </div>
      }
    ...
    {!isLoad && <Confirm header={"حذف"} massage={"هل تريد حذف مخزن؟ "} handelDelete={handelDelete} element={element} color={"danger"} icon={faTrashCan} textBtn={"حذف"}  />}
